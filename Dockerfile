...
RUN apt-get update && apt-get install -y sudo git zip wget
...
RUN apt-get update && apt install -y build-essential gcc-mingw-w64-x86-64-win32 python3-pip python3-tk
RUN apt-get update && apt install -y dnscat2 evil-winrm villain
RUN python3 -m pip install pwncat-cs --break-system-packages
