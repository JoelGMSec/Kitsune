...
def pwncat(app, host, port, name, session, restart):
    ...
    session_data = pexpect.spawn(f'python3.11 $(which pwncat-cs) 0.0.0.0 {port}', cwd=pwncat_path, echo=False, use_poll=True) 
    session_data.timeout = 1
...
def villain(app, host, port, name, session, restart):
    ...
    session_data = pexpect.spawn(f'python3 Villain.py -i -s -q -n {port} -p 33333 -x 44444 -f 55555', cwd=villain_path, echo=False, use_poll=True)
    session_data.timeout = 1
