...
def connect_pwncat(app, params, method, session, restart):
    ...
    if method:
        ...
        host = params.split("@")[1]
        pasw = method
        session_data = pexpect.spawn(f'pwncat-cs ssh://{user}:{pasw}@{host}', cwd=pwncat_path, echo=False, use_poll=True)  
    
    else:
        session_info["Listener"] = "BIND"
        session_data = pexpect.spawn(f'pwncat-cs connect://{params}', cwd=pwncat_path, echo=False, use_poll=True)  
