...
def open_settings(app):
    ...
    app.settings_window.title("Nekomancer")
    app.settings_window.focus_force()
    app.settings_window.resizable(False, False)
    image_frame = tk.Frame(app.settings_window)
    image_frame.grid(row=0, column=1, padx=(0, 20), pady=(20, 20))
    ...
    selected_value = tk.StringVar(value=settings["settings"].get("nekomancer", "All Sessions"))
...
def save_and_close(app, selected_value):
    ...
    # Asegurar que "settings" existe y actualizar solo sus valores
    if "settings" not in data:
        data["settings"] = {}

    data["settings"]["theme"] = app.theme_var.get()
    data["settings"]["nekomancer"] = selected_value.get()

    # Guardar el JSON actualizado sin eliminar otras claves
